...
export default  class Main extends Component<IProps, IState>{
  ...
  public refer = () => {
    ...
    axios.post("http://www.test.com/information/select.php",referData).then((res: any) => {  
      if (res.data.code === 200) {
        this.setState({
  ...
  public openModal = (record: any) => {
      this.props.history.push({ pathname: '/admin/NewsConstruction/information/edit', data:{list:record.list,title:'修改团委信息'}})
  }
  public openAudit =  (record:any) => {
    this.props.history.push({ pathname: '/admin/NewsConstruction/information/audit',data:{list:record.list,}})
  }
    //删除用户数据
    public deleteData = (record: any) => { 
      ...
      axios.post("http://www.test.com/information/delete.php",deleteData).then((res: any) => {
        if (res.data.code === 200) { 
  ...
  //新增团委信息
    public add = () => {
        this.props.history.push({ pathname: '/admin/NewsConstruction/information/edit', data: {title:'新增团委信息发布'}})
    }
    render() {
        const columns:any= [
            {
        ...
        return (
            <Card title="学校团委信息" extra={<Button type="primary" size="small" onClick={ this.add}>新增</Button>}>
                <div style={{
              marginBottom: '20px',
              marginLeft:'40px'
            }}>
